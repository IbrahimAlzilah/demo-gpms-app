...
export function ProjectProgressTracker({ projectId }: ProjectProgressTrackerProps) {
  ...
  const saveNote = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast(t('supervisor.noteSaved') || 'تم حفظ الملاحظة بنجاح', 'success')
      setNotes('')
    },
    onError: () => {
      showToast(t('supervisor.noteError') || 'فشل حفظ الملاحظة', 'error')
    },
  })

  const handleSaveNote = async () => {
    if (!notes.trim()) {
      showToast(t('supervisor.noteRequired') || 'يرجى إدخال ملاحظة', 'warning')
      return
    }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('supervisor.addNotes') || 'إضافة ملاحظات'}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="notes">{t('supervisor.notes') || 'الملاحظات'}</Label>
            <Textarea
              id="notes"
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder={t('supervisor.notesPlaceholder') || 'أدخل ملاحظاتك حول تقدم المشروع'}
              rows={4}
            />
          </div>
          <Button 
      ...
      {supervisorNotes && supervisorNotes.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <MessageSquare className="h-5 w-5 text-primary" />
              {t('supervisor.previousNotes') || 'الملاحظات السابقة'}
            </CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  {note.studentReplies && note.studentReplies.length > 0 && (
                    <div className="mt-3 space-y-2 border-t pt-3">
                      <p className="text-xs font-medium text-muted-foreground">
                        {t('supervisor.studentReplies') || 'ردود الطلاب'}
                      </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t('supervisor.evaluations') || 'التقييمات'}
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {grades.map((grade) => (
                <div key={grade.id} className="p-4 bg-muted rounded-lg border">
                  <p className="font-medium">
                    {t('supervisor.grade') || 'الدرجة'}: {grade.supervisorGrade?.score || t('common.notSet') || 'غير محدد'} /{' '}
