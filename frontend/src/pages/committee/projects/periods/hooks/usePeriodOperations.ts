...
/**
 * Hook for activating a period
 */
export function useActivatePeriod() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) => periodService.update(id, { isActive: true }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["periods"] });
      queryClient.invalidateQueries({ queryKey: ["committee-periods-table"] });
...
/**
 * Hook for deactivating a period
 */
export function useDeactivatePeriod() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (id: string) => periodService.update(id, { isActive: false }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["periods"] });
      queryClient.invalidateQueries({ queryKey: ["committee-periods-table"] });
      queryClient.invalidateQueries({ queryKey: ["timeWindows"] });
