...
export function useProposalsEditBatch(onSuccess?: () => void) {
  ...
  const { isPeriodActive: isRegistrationPeriod } = usePeriodCheck(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const message = getApiErrorMessage(error, t, "proposal.updateError");
      toastError(message);
    } finally {
