...
import { User, Users, Calendar, FileText, CheckCircle2, MessageSquare } from 'lucide-react'
...
interface ProposalSubmissionViewProps {
  submissionId?: string
...
export function ProposalSubmissionView({
  ...
  const { data: submissionData, isLoading } = useQuery({
    queryKey: ['proposal-submission', submissionId],
    queryFn: () => submissionService.getSubmission(),
    enabled: open && !!submissionId && !submissionProp,
  })

  const submission = submissionProp || submissionData

  if (!open || isLoading || (!submission && !submissionId)) {
    return null
  }
