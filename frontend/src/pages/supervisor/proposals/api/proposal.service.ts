...
export const proposalService = {
  ...
  createBatch: async (
    ...
    const response = await apiClient.post<{ data: Proposal[] }>(
      "/supervisor/proposals/batch",
      {
  ...
  getStudentGroups: async (): Promise<
    Array<{
      id: string;
      name: string;
      code: string;
  ...
  > => {
    ...
    }>("/supervisor/proposals/student-groups");
    return response.data.data || [];
  },
  ...
  assignToGroup: async (
    ...
    const response = await apiClient.post<{ data: Proposal }>(
      `/supervisor/proposals/${proposalId}/assign`,
      {
  ...
  requestAssignment: async (
    ...
    const response = await apiClient.post<{ data: Proposal }>(
      `/supervisor/proposals/${proposalId}/request-assignment`,
      {
        student_group_id: studentGroupId,
        notes: notes || null,
      },
  ...
  delete: async (id: string): Promise<void> => {
    await apiClient.delete(`/supervisor/proposals/${id}`);
  },
};
