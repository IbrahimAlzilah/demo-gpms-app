...
**الصلاحيات الأساسية للطالب:**
- تقديم مقترحات مشاريع خلال نافذة تقديم المقترحات
- تصفح المشاريع المعتمدة والمعلنة من قبل لجنة المشاريع
...
- **المتطلبات المسبقة**: 
  - نافذة `grade_approval` نشطة
  - الدرجة لديها `final_grade` محسوبة
  - الدرجة غير معتمدة مسبقاً (`is_approved = false`)
- **التدفق الأساسي**:
  1. اللجنة تعرض الدرجات غير المعتمدة التي لديها `final_grade`
  2. اللجنة تراجع الدرجة (supervisor grade, committee grade, final grade)
  3. اللجنة تعتمد الدرجة
  4. النظام يحدث `is_approved = true` و `approved_by` و `approved_at`
  5. النظام يرسل إشعار للطالب
